                                ...
                                <div class="row">
                                    <div class="col-md-8">
                                        ...
                                        <select name="classtype" id="classtype" class="form-control" required>
                                          ...
                                          <?php
                                          $sel_query=mysqli_query($conn,"SELECT * FROM `classtype` WHERE status='Active' AND id=3");
                                        ...
                                        <div class="form-group">
                                            <label>
                                                ...
                                                <input type="text" name="cat_symbol" placeholder="Enter Storage .." class="form-control" value="<?php if(isset($_POST['cat_symbol'])){ echo $_POST['cat_symbol'];}else{ echo $varq['symbol'];}?>" required>
                                            </label>
                                        </div>
                                        <div class="form-group">
...
//                        print_r($_POST);                            print_r($_FILES); 
                        if(isset($_POST['submit']))
                        {
                         ...
                         $sel_query=mysqli_query($conn,"SELECT * FROM `size_class` WHERE classtype_id=$classtypeId AND trash='No' AND symbol='".$symbol."' AND id=!'".$sizeId."'");
                            ...
                            {
                                //echo "<script>";
                                echo '<div id="snackbar">This Storage Class is already added..</div>';
                           ...
                           $query = "UPDATE size_class
                               ...
                               $query=mysqli_query($conn,$query);
                                if($query)
                                {
                                    ...
                                    echo '<div id="snackbar">Storage Class Edited successfully..</div>';
                                    ...
                                    echo"var delay = 1000;setTimeout(function(){ window.location = 'view-classtype-list.php?id=3'; }, delay);";
                                    echo "</script>";
                                }
                                else
                                {
                                    echo '<div id="snackbar">Your Storage Class Not Edited..</div>';
                                    echo "<script> var x = document.getElementById('snackbar');x.className = 'show';setTimeout(function(){ x.className = x.className.replace('show', ''); }, 3000);</script>";
                                }
                        }
                    }
