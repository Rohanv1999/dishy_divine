                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade show active" id="tab01">
                            <form action="" method="post" enctype="multipart/form-data" name="form">  
                                <?php
                                $pquery=mysqli_query($conn,"SELECT * FROM `disclaimer_title` WHERE id=$tid"); //products select query
                                $pdata=mysqli_fetch_array($pquery);

                                $des_query=mysqli_query($conn,"SELECT * FROM `disclaimer_desc` WHERE title_id=$tid"); //description select query ?>
...
{
    $title=$_POST['title'];
    $up=mysqli_query($conn,"UPDATE `disclaimer_title` SET `title`='$title' WHERE id=$tid");
    ...
    {
        $auid=$rid[$key];
        $update=("UPDATE `disclaimer_desc` SET `description` = '$value' WHERE title_id=$tid AND id = $auid");
    ...
    if(isset($_POST['add_description']))
    {
        ...
        {
            $query=mysqli_query($conn,"INSERT INTO `disclaimer_desc`(`title_id`, `description`) VALUES ('$tid','$value')");
        }
    }
    if($des_query)
    {  
        ...
        echo"var delay = 1000;setTimeout(function(){ window.location = 'Disclaimer-view.php'; }, delay);";
        echo "</script>";
    }
    else
    {
