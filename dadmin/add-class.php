    ...
    {
        $id=$_GET['id'];
      $classtypeName=mysqli_fetch_assoc(mysqli_query($conn,"SELECT name FROM classtype WHERE id=$id"))['name'];  
                ...
                <div class="panel">
                    <div class="panel-content">
                        ...
                        <form action="" method="post" id="formWizard" class="form--wizard" enctype="multipart/form-data">
                            <h3>ADD <?=strtoupper($classtypeName); ?></h3>
                            <section>
                                <div class="row">
                                    <div class="col-md-8">
                                        ...
                                        <div class="form-group">
                                            <label>
                                                ...
                                                <input type="text" name="cat_name" placeholder="Enter <?=ucfirst(rtrim($classtypeName,'s')); ?> Name.." class="form-control" value="<?php if(isset($_POST['cat_name'])){ echo $_POST['cat_name'];}?>" required>
                                            </label>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label>
                                                ...
                                                <input type="text" name="cat_symbol" placeholder="Enter <?=ucfirst(rtrim($classtypeName,'s')); ?> Symbol.." class="form-control" value="<?php if(isset($_POST['cat_symbol'])){ echo $_POST['cat_symbol'];}?>" required>
                                            </label>
                                        </div>
                                        <div class="form-group">
...
//                        print_r($_POST);                       print_r($_FILES); 
                        ...
                        {
                            $name=strtolower($_POST['cat_name']);
                            $classtypeId=$_POST['classtype'];
                            $class=ucfirst(rtrim($classtypeName,'s'));
                            $sel_query=mysqli_query($conn,"SELECT * FROM `size_class` WHERE classtype_id=$classtypeId AND name='".$name."' AND trash='No'");
                            ...
                            else
                            {
                               $query=mysqli_query($conn,"INSERT INTO `size_class`(`classtype_id`,`name`,`symbol`) VALUES ('$classtypeId','$name','".$_POST['cat_symbol']."')");
                                if($query)
                                {
                                    ...
                                    echo '<div id="snackbar"><?=$class; ?> Class add successfully..</div>';
                                ...
                                {
                                    echo '<div id="snackbar">Your <?=$class; ?> Class Not Added..</div>';
                                    echo "<script> var x = document.getElementById('snackbar');x.className = 'show';setTimeout(function(){ x.className = x.className.replace('show', ''); }, 3000);</script>";
                                }
